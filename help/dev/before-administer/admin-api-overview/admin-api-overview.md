...
>
>Admin-API:erna och profil-API:erna kallas ofta tillsammans (&quot;Admin- och Profile-API:er&quot;), men kan också hänvisas till separat (&quot;Admin-API:er&quot; och&quot;Profil-API:er&quot;). Rekommendations-API:t är en specifik implementering av ett [!DNL Target]-administratörs-API.
...
* [Rekommendationer](https://experienceleague.adobe.com/docs/target/using/activities/recommendations-activity.html?lang=sv-SE)
...
Batch-API:t innehåller en array med logiska HTTP-begäranden som representeras av JSON-matriser - varje begäran har en metod (som motsvarar HTTP-metoden GET/PUT/POST/DELETE osv.), en relativeUrl (den del av URL:en som skapas efter admin/rest/), en valfri rubrikmatris (som motsvarar HTTP-huvuden) och ett valfritt innehåll (för POST- och PUT-begäranden). Batch-API:t returnerar en array med logiska HTTP-svar representerade som JSON-arrayer - varje svar har en statuskod, en valfri rubrikarray och en valfri brödtext (som är en JSON-kodad sträng). Om du vill göra grupperade begäranden skapar du ett JSON-objekt som beskriver varje enskild åtgärd som ska utföras. Antalet tillåtna operationer är 256 (från 0 till 255).
...
För att länka ihop två batchåtgärder anger du ID:t för den nödvändiga åtgärden i den beroende åtgärden, till exempel: &quot;beroendeOnOperationId&quot; : 5. ID:n för skapade resurser via POST-begäranden för gruppåtgärder kan också användas i beroende åtgärder både i &quot;relativeUrl&quot; och &quot;body&quot;.
...
| --- | --- | --- | --- |
| brödtext | brödtext för HTTP-batchåtgärd. ignoreras för alla åtgärder förutom POST och PUT. kan referera till ID:n från tidigare batchåtgärder, till exempel: &quot;offerId&quot;: &quot;{operationIdResponse:0}&quot;, &quot;segmentId&quot;: &quot;{operationIdResponse:1}&quot; | ska vara en giltig JSON; om en operationIdResponse refereras ska det refererade operationId-svaret vara ett giltigt ID och metoden för den åtgärden ska vara POST | tomt objekt {} |
...
| method | HTTP-metod som ska användas. Tillgängliga alternativ: GET, POST, PUT, PATCH, DELETE | Endast GET, POST, PUT, PATCH, DELETE tillåts |  |
...
| relativeUrl | relativ URL för admin rest API, delen efter /admin/rest/. Kan innehålla frågesträngsparametrar som: &quot;/v2/campaign?limit=10&amp;offset=10&quot;. kan referera till URL:er med ID:n från tidigare gruppåtgärder, till exempel: &quot;/v1/offers/{operationIdResponse:0}&quot;. Om frågeparametrar skickas måste de vara URL-kodade. | ska börja med / (be relative); endast nya giltiga JSON API:er stöds. Om relativeURL är ogiltig returneras ett 404-svar för en viss åtgärd. Om en operationIdResponse refereras ska operationId-svaret som refereras vara ett giltigt ID och metoden för den åtgärden ska vara POST |  |
...
| --- | --- |
| operationId | Åtgärds-ID som används för att identifiera en åtgärd bland andra åtgärder, samma ID som det har skickats i POST-begäran. |
