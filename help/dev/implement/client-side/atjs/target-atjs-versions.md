...
* Ett problem som gjorde att svarstoken inte kunde skickas har korrigerats `at-content-rendering-failed` händelser.
...
* Korrigerade ett problem som felaktigt utlöste `at-content-rendering-succeeded` anpassad händelse när inga erbjudanden återges. Den rätta händelsen, `at-content-rendering-no-offers`, aktiveras nu.
* Tillagd `eventToken` och `responseTokens` till felobjekt för `at-content-rendering-failed` anpassad händelse.
